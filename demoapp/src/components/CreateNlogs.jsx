...
export default function CreateNlogs() {
  ...
  const handleAddLogs = async () => {
    const response = await addNumberofLogstoDatabase(number, campLocation);
    if (response && response.message === "Success") {
      alert(`Successfully added ${number} logs`);
    } else {
      alert("Error adding logs: " + (response.error || "Unknown error"));
    }
  };
